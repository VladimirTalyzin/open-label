import { defineConfig } from 'vite'

export default defineConfig({
  root: 'src',
  publicDir: '../public',
  build: {
    outDir: '../dist',
    emptyOutDir: true,
  },
  server: {
    port: 3000,
    proxy: {
      '/auth': 'http://localhost:8001',
      '/admin': 'http://localhost:8001',
      '/add_project': 'http://localhost:8001',
      '/get_projects_list': 'http://localhost:8001',
      '/get_project_data': 'http://localhost:8001',
      '/set_project_name': 'http://localhost:8001',
      '/set_project_labels': 'http://localhost:8001',
      '/set_prediction_url': 'http://localhost:8001',
      '/upload_image': 'http://localhost:8001',
      '/get_images_list': 'http://localhost:8001',
      '/get_png_mask': 'http://localhost:8001',
      '/upload_png_mask': 'http://localhost:8001',
      '/get_vector_mask': 'http://localhost:8001',
      '/upload_vector_mask': 'http://localhost:8001',
      '/undo_png_mask': 'http://localhost:8001',
      '/image': 'http://localhost:8001',
      '/get_preview': 'http://localhost:8001',
      '/regenerate_previews': 'http://localhost:8001',
      '/rebuild_all_previews': 'http://localhost:8001',
      '/regenerate_skeleton_svgs': 'http://localhost:8001',
      '/predict': 'http://localhost:8001',
      '/predict_with_crop': 'http://localhost:8001',
      '/delete_image': 'http://localhost:8001',
      '/delete_project': 'http://localhost:8001',
      '/update_project': 'http://localhost:8001',
      '/set_project_type': 'http://localhost:8001',
      '/set_skeleton_template': 'http://localhost:8001',
      '/get_skeleton_data': 'http://localhost:8001',
      '/upload_skeleton_data': 'http://localhost:8001',
      '/get_skeleton_svg': 'http://localhost:8001',
      '/get_skeleton_mask': 'http://localhost:8001',
      '/upload_skeleton_mask': 'http://localhost:8001',
      '/export_dataset': 'http://localhost:8001',
      '/download_dataset': 'http://localhost:8001',
      '/export_project': 'http://localhost:8001',
      '/import_project': 'http://localhost:8001',
      '/download_export': 'http://localhost:8001',
      // --- Multi-channel (yolo-skeleton) ---
      '/set_project_channels': 'http://localhost:8001',
      '/set_channel_skeleton_template': 'http://localhost:8001',
      '/upload_image_channel': 'http://localhost:8001',
      '/image_channel': 'http://localhost:8001',
      '/channel_images': 'http://localhost:8001',
      '/predict_skeleton': 'http://localhost:8001',
      '/get_skeleton_data_channel': 'http://localhost:8001',
      '/upload_skeleton_data_channel': 'http://localhost:8001',
      '/get_skeleton_svg_channel': 'http://localhost:8001',
      '/get_channel_preview': 'http://localhost:8001',
      '/set_preview_channel': 'http://localhost:8001',
    },
  },
})
